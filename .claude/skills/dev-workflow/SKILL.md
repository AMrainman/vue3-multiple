...
- **自动模式（默认）**：跳过所有确认，一键执行完整流程
- **交互模式（`auto: false`）**：在每个关键步骤暂停，等待用户确认

### 交互模式调用示例

当用户需要逐步确认时，使用 `auto: false` 参数：
...
| 步骤        | 自动模式（默认） | 交互模式         |
| ----------- | ---------------- | ---------------- |
| Lint 检查后 | 直接执行 `--fix` | 询问是否修复     |
| Commit 信息 | 直接提交         | 展示供确认修改   |
| Push 前     | 直接推送         | 确认推送         |
| 创建 PR 前  | 直接创建 PR      | 询问是否创建     |
...
执行破坏性或不可逆操作（push、创建 PR）前，交互模式下需先与用户确认。
